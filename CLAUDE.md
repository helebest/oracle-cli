...
uv run oci-vm setup openclaw                  # Deploy OpenClaw agent (loopback-only web UI)
...
- **Hermes** (AI agent) — `nousresearch/hermes-agent` from Docker Hub (multi-arch: amd64 + arm64). Custom Dockerfile adds `gh` and `vim`. Runs `gateway run` for messaging platforms. Has optional HTTP API on port 8642 (disabled by default, enable via `API_SERVER_ENABLED=true`). Resource limits: 8G memory, `cpus: "2.0"` (must not exceed the VM's OCPU count — VM.Standard.A1.Flex is 2 OCPUs — or Docker rejects container (re)create). **iMessage via Photon**: the Dockerfile bakes the Photon Node sidecar (`spectrum-ts`) into the image at build time — `npm ci` in `/opt/hermes/plugins/platforms/photon/sidecar` as root, because v2026.6.19 makes `/opt/hermes` immutable so the runtime non-root `hermes` user can't `hermes photon install-sidecar` itself. Node (v22) is already in the base image. To activate, run the one-time interactive setup inside the container (it does the Apple device-code login + assigns an iMessage line; sidecar deps are already present, so pass `--skip-sidecar-install`): `docker exec -it hermes hermes photon setup --phone +1555... --skip-sidecar-install`, then `docker exec -it hermes hermes gateway restart`. Creds persist to `$HERMES_HOME` (`/opt/data`, the mounted volume), and the channel auto-enables once `PHOTON_PROJECT_ID`/`PHOTON_PROJECT_SECRET` are stored. Approve senders with `hermes pairing approve photon <CODE>`. No Mac required — Photon is a managed Apple-line service.
...
- **OpenClaw** (AI agent) — `ghcr.io/openclaw/openclaw:2026.6.1` from GHCR (multi-arch: amd64 + arm64). Gateway runs `node dist/index.js gateway --bind loopback --port 18789` so the web UI is **loopback-only** (`127.0.0.1:18789`) — invisible to public internet *and* to the tailnet. Reach it from a tailnet peer via SSH tunnel (`ssh -L 18789:127.0.0.1:18789 ubuntu@oracle-vm`). Telegram + Feishu channels work without inbound exposure: Telegram uses long-poll, Feishu defaults to WebSocket. Persistent state in two host-mounted dirs: `./data/config` → `/home/node/.openclaw` and `./data/workspace` → `/home/node/.openclaw/workspace` (uid 1000:1000, the `node` user inside the image). Secrets in `docker/openclaw/.env` (gitignored): `OPENCLAW_GATEWAY_TOKEN`, `OPENAI_API_KEY`, `TELEGRAM_BOT_TOKEN`, `FEISHU_APP_ID` / `FEISHU_APP_SECRET` / `FEISHU_VERIFICATION_TOKEN` / `FEISHU_ENCRYPT_KEY`. After deploy, complete one-time pairing: `docker exec -it openclaw openclaw pairing approve telegram <CODE>` (Telegram) and `docker exec -it openclaw openclaw channels login --channel feishu` (Feishu QR). Resource limits: 4G memory, 2.0 CPU.
