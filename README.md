...
| OpenClaw | `ghcr.io/openclaw/openclaw:2026.6.1` agent gateway. Web UI loopback-only (127.0.0.1:18789); reach via `ssh -L 18789:127.0.0.1:18789 ubuntu@oracle-vm`. Telegram + Feishu channels via outbound (no inbound port). Persistent state at `data/{config,workspace}/`. | `docker/openclaw/` |
